...
- (NSArray *)query:(QNDomain *)domain networkInfo:(QNNetworkInfo *)netInfo error:(NSError *__autoreleasing *)error {
    ...
    if (r != 0) {
        if (error != nil) {
            *error = [[NSError alloc] initWithDomain:@"server.dns" code:kQNDomainSeverError userInfo:nil];
        }
        return nil;
    }
    ...
    if (error != nil) {
        *error = [[NSError alloc] initWithDomain:@"server.dns" code:NSURLErrorDNSLookupFailed userInfo:nil];
    }
    return nil;
}
